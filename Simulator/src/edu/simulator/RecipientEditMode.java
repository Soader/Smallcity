...
public class RecipientEditMode extends EditMode {
	...
	public RecipientEditMode(Simulator simulator) {
		...
		while (Mouse.next()) {
			if (Mouse.getEventButtonState()) {
				if (Mouse.getEventButton() == 0) {
					...
					if (!skip) {
						pressedRecipient(Mouse.getEventX(), Mouse.getEventY());
					}
					skip = false;
				}
			} else {
				if (Mouse.getEventButton() == 0) {
					...
					if (!skip) {
						releasedRecipient(Mouse.getEventX(), Mouse.getEventY());
					}
					skip = false;
				}
			}
	...
	private void pressedRecipient(int mouseX, int mouseY) {
	...
	private void releasedRecipient(int mouseX, int mouseY) {
		int i = mouseX / Block.SIZE;
		int j = mouseY / Block.SIZE;
		if (creating) {
			if (i == RecX && j == RecY) {
				Recipient rec = new Recipient(i * Block.SIZE, j
						* Block.SIZE, RecSX * Block.SIZE, RecSY * Block.SIZE,
						simulator);
				simulator.recipients.add(rec);
				simulator.buildings.add(rec);
	...
	public void render() {
		...
		if(!isOut)Draw.Rect(RecX * Block.SIZE, RecY * Block.SIZE, RecSX * Block.SIZE,
				RecSY * Block.SIZE, EnumSim_Texture.RECIPIENT.texture);
