...
public class SupplierEditMode extends EditMode {
	...
	public SupplierEditMode(Simulator simulator) {
		...
		while (Mouse.next()) {
			if (Mouse.getEventButtonState()) {
				if (Mouse.getEventButton() == 0) {
					...
					if (!skip) {
						pressedSupplier(Mouse.getEventX(), Mouse.getEventY());
					}
					skip = false;
				}
			} else {
				if (Mouse.getEventButton() == 0) {
					...
					if (!skip) {
						releasedSupplier(Mouse.getEventX(), Mouse.getEventY());
					}
					skip = false;
				}
			}
	...
	private void pressedSupplier(int mouseX, int mouseY) {
	...
	private void releasedSupplier(int mouseX, int mouseY) {
		int i = mouseX / Block.SIZE;
		int j = mouseY / Block.SIZE;
		if (creating) {
			if (i == RecX && j == RecY) {
				Supplier sup = new Supplier(i * Block.SIZE, j
						* Block.SIZE, RecSX * Block.SIZE, RecSY * Block.SIZE,
						 simulator);
				simulator.suppliers.add(sup);
				simulator.buildings.add(sup);
	...
	public void render() {
		...
		if(!isOut)Draw.Rect(RecX * Block.SIZE, RecY * Block.SIZE, RecSX * Block.SIZE,
				RecSY * Block.SIZE, EnumSim_Texture.SUPPLIER.texture);
